...
from gpt.utils import chunked_cross_entropy, get_default_supported_precision
from gpt.speed_monitor import SpeedMonitorCallback, estimate_flops, measure_flops
...
class LightningGPTModule(L.LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        #self.log("train_loss", loss, on_step=True,logger = True, on_epoch=False, prog_bar=True)
        return loss
...
def main(devices: int = 1, precision: Optional[str] = None) -> None:
    ...
    speed_monitor = SpeedMonitorCallback(
        length_fn=lambda batch: batch[0].size(1), batch_size=micro_batch_size, window_size=25, time_unit="seconds"
    )
    ...
    trainer = L.Trainer(
        ...
        precision=precision,
        logger=logger,
        enable_checkpointing=always_save_checkpoint,
    ...
    if trainer.global_rank == 0 and always_save_checkpoint:
